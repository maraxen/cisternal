"""Tests for M10.1 / M10.2 / M10.4 ``cisternal telemetry doctor``."""
...
from cisternal.probe.telemetry_doctor import (
    build_doctor_report,
...
from cisternal.telemetry.pipeline import resolve_log_dir_from_env
...
def test_resolve_log_dir_from_env_precedence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CISTERNAL_LOG_DIR", raising=False)
    ...
    assert default.parent.name == ".cisternal"
    ...
    monkeypatch.setenv("CISTERNAL_LOG_DIR", "/cisternal/logs")
    assert resolve_log_dir_from_env() == Path("/cisternal/logs")


def test_doctor_consumer_matrix(monkeypatch: pytest.MonkeyPatch) -> None:
    """AC-M10.1-1: per-consumer enabled state from CISTERNAL_TELEMETRY."""
...
def test_doctor_all_consumers_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CISTERNAL_TELEMETRY", "all")
    report = format_doctor_report()
...
def test_doctor_log_dir_and_otlp(
    ...
    monkeypatch.setenv("CISTERNAL_LOG_DIR", str(log_dir))
    monkeypatch.setenv("CISTERNAL_OTLP_ENDPOINT", "http://localhost:4317")
    monkeypatch.setenv("CISTERNAL_OTLP_PROTOCOL", "grpc")

    report = format_doctor_report()
    assert f"resolved: {log_dir}" in report
    assert "writable: yes" in report
    assert "CISTERNAL_OTLP_ENDPOINT: http://localhost:4317" in report
    assert "CISTERNAL_OTLP_PROTOCOL: grpc" in report
...
def test_doctor_pipeline_inactive() -> None:
    ...
    report = format_doctor_report()
    assert "inactive (expected unless cisternal.init() ran)" in report
...
def test_doctor_cli_invocation(
    ...
    assert "cisternal telemetry doctor" in out
...
def test_doctor_help_references_runbook(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    out = capsys.readouterr().out
    assert "cisternal-telemetry.md" in out
...
def test_cli_still_fastmcp_free() -> None:
    ...
    import cisternal.cli as cli_mod
...
def test_build_doctor_report_telemetry_gate_warn_for_invalid_token(
    ...
    monkeypatch.setenv("CISTERNAL_TELEMETRY", "bogus")
    report = build_doctor_report()
...
def test_resolve_doctor_strict_mode_from_env(
    ...
    monkeypatch.setenv("CISTERNAL_DOCTOR_STRICT", "true")
...
def test_effective_check_status_promotion() -> None:
    from cisternal.probe.telemetry_doctor import DoctorCheck
...
def test_doctor_cli_json_only_stdout(
    ...
    assert exc_info.value.code == 0
    out = capsys.readouterr().out
    assert "cisternal telemetry doctor" not in out
...
def test_doctor_cli_strict_env_exit_one(
    ...
    monkeypatch.delenv("CISTERNAL_TELEMETRY", raising=False)
    monkeypatch.setenv("CISTERNAL_DOCTOR_STRICT", "1")
    from cisternal.cli import app
...
def test_ci_preflight_env_passes_strict(
    ...
    """AC-M10.3: export-dogfood doctor step env (CISTERNAL_TELEMETRY=all + --strict)."""
    monkeypatch.setenv("CISTERNAL_TELEMETRY", "all")
    monkeypatch.delenv("CISTERNAL_DOCTOR_STRICT", raising=False)
    report = build_doctor_report()
...
def test_resolve_doctor_consumer_cli_overrides_env(
    ...
    """AC-M10.4: CLI --consumer wins over CISTERNAL_DOCTOR_CONSUMER."""
    monkeypatch.setenv("CISTERNAL_DOCTOR_CONSUMER", "bathos")
...
def test_resolve_doctor_consumer_from_env(
    ...
    monkeypatch.setenv("CISTERNAL_DOCTOR_CONSUMER", "Xperiri")
