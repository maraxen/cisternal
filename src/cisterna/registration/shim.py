...
TBD-M2-3 — async tool invoked via CLI inside a running event loop:
    ...
    Resolution strategy (chosen for M2):
        ...
        is async, a :class:`cisterna.registration.errors.CisternaWireError` is
...
async def dispatch(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    ...
    produced by :func:`cisterna.registration.compose.compose_mcp_callable`.
...
def cli_dispatch(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    ...
    function raises :class:`~cisterna.registration.errors.CisternaWireError`
    ...
    Raises:
        CisternaWireError: If *fn* is async and a running event loop is
