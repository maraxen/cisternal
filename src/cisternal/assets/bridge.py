...
from cisternal.assets.bundle import AssetBundle, HookSpecAsset, McpAsset
...
def asset_bundle_to_praxia_json(bundle: AssetBundle) -> dict[str, Any]:
    """Map cisternal AssetBundle to PraxiaBundle JSON for praxia-agent-assets."""
    return {
        "metadata": {
...
def resolve_bundle_hash_bin(bin_path: str | None = None) -> str | None:
    ...
    raw = (bin_path or os.environ.get("CISTERNAL_PRAXIA_ASSETS_BIN", "")).strip()
    return raw or None
...
def rust_surface_digest(
    ...
    *,
    ...
    if resolved is None:
        msg = "CISTERNAL_PRAXIA_ASSETS_BIN is unset; cannot run rust parity digest"
