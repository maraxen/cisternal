        ...
        while not self._stop_event.is_set():
            ...
            for exporter in self._exporters:
                try:
                    ...
                    print(
                        f"[cisternal] Exporter {type(exporter).__name__} raised: {e}",
                        file=sys.stderr,
                    )
    ...
    def shutdown(self, timeout: float = 2.0) -> None:
        ...
        for exporter in self._exporters:
            try:
                ...
                print(
                    f"[cisternal] Exporter.flush() failed: {e}",
                    file=sys.stderr,
                )
        ...
        try:
            ...
            print(
                f"[cisternal] Listener thread shutdown failed: {e}",
                file=sys.stderr,
            )
        ...
        for exporter in self._exporters:
            try:
                ...
                print(
                    f"[cisternal] Exporter.close() failed: {e}",
                    file=sys.stderr,
                )
...
def resolve_log_dir_from_env() -> Path:
    ...
    ``CISTERNAL_LOG_DIR`` > ``BTH_LOG_DIR`` > ``CTXP_LOG_DIR`` > ``~/.cisternal/logs``
    """
    import os

    raw = (
        os.getenv("CISTERNAL_LOG_DIR")
        or os.getenv("BTH_LOG_DIR")
        or os.getenv("CTXP_LOG_DIR")
    )
    if raw is None:
        return Path.home() / ".cisternal" / "logs"
    return Path(raw)
...
def init_pipeline(
    ...
    Spec §3.2: log_dir env resolution: CISTERNAL_LOG_DIR > BTH_LOG_DIR > CTXP_LOG_DIR > ~/.cisternal/logs
    ...
    with _pipeline_lock:
        ...
        try:
            ...
            print(
                f"[cisternal] Warning: cannot write to {log_dir}; falling back to tempdir",
                file=sys.stderr,
            )
